...
public class AndroidAudioPlayer : AudioPlayer {
    ...
    public override Task<double> GetTotalTime() {
    ...
    public override Task Play(string filePath) {
        _player.Stop();
        _player.Release();
        _player.Dispose();
        _player = new();
        ...
        _isPaused = false;
        _isStopped = false;
        Uri uri = UriUtility.GetUriFomItemPath(filePath) ?? throw new NullReferenceException();
    ...
    public override Task Seek(double miliSeconds) {
    ...
    public override Task SeekTo(double miliSeconds) {
